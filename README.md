...
| Docker Volumes vs Bind Mounts | Persistent Docker storage backed by host directories | WordPress files and MariaDB data are persisted outside their containers so that recreating a container does not require reinstalling WordPress or rebuilding the database. Host-backed storage also keeps the project data in a predictable local location, configurable through `DATA_ROOT_PATH` and the directory names `DATA_DRIVE_DIR` / `WEB_DRIVE_DIR` in `srcs/.env`. |
